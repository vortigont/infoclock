...
void create_parameters(){
    ...
    embui.section_handle_add(FPSTR(T_CLOCK), block_page_clock);             // generate "clock" page
    ...
    embui.section_handle_add(FPSTR(T_UPD_WEATHER), upd_weather);            // обновить погоду
...
void block_menu(Interface *interf, JsonObject *data){
    ...
    interf->option(FPSTR(T_CLOCK), FPSTR(T_DICT[lang][TD::D_Clock]));
...
void block_page_clock(Interface *interf, JsonObject *data){
    if (!interf) return;
    interf->json_frame_interface();
    interf->json_section_main(FPSTR(T_CLOCK), FPSTR(T_DICT[lang][TD::D_Clock]));
    ...
    interf->button_submit(FPSTR(T_UPD_WEATHER), F("Обновить погоду"), FPSTR(T_GRAY));
    interf->button_submit("mxreset", F("Reset matrix"), FPSTR(T_GRAY));
